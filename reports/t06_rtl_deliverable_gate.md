...
Root: `E:\B题改进\fpga_stage1\rtl\geometry\cordic_sincos.v`
Delivery ready: `False`
Summary: **272 error(s)**, **0 strict warning(s)**
...
- Status: `failed`
- Severity: `WARNING`
- Location: `cordic_sincos.v:135`
- Evidence:
- node_kind: `verilog_rtl`
- detail: if(iteration_count == 5'd17)
- source_excerpt: `if(iteration_count == 5'd17)`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: reduced_phase = 18'd131072 - phase_i;
- source_excerpt: `reduced_phase = 18'd131072 - phase_i;`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: reduced_phase = phase_i - 18'd131072;
- source_excerpt: `reduced_phase = phase_i - 18'd131072;`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: reduced_phase = 18'd0 - phase_i;
- source_excerpt: `reduced_phase = 18'd0 - phase_i;`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: x_value <= x_value - y_shifted;
- source_excerpt: `x_value <= x_value - y_shifted;`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: y_value <= y_value + x_shifted;
- source_excerpt: `y_value <= y_value + x_shifted;`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: z_value <= z_value - atan_step;
- source_excerpt: `z_value <= z_value - atan_step;`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: iteration_count <= iteration_count + 1'b1;
- source_excerpt: `iteration_count <= iteration_count + 1'b1;`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
### VG125: 字面量声明为 36 位，但实际值只需要 2 位。
- Status: `failed`
- Severity: `BLOCKER`
- Location: `cordic_sincos.v:56`
- Evidence:
- node_kind: `verilog_rtl`
- detail: 36'sd2
- source_excerpt: `36'sd2`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=x_rounded; child_output=cordic_sincos.x_rounded; operation_count=8; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=x_rounded; child_output=cordic_sincos.x_rounded; operation_count=8; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=x_rounded; child_output=cordic_sincos.x_rounded; operation_count=8; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=y_rounded; child_output=cordic_sincos.y_rounded; operation_count=8; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=y_rounded; child_output=cordic_sincos.y_rounded; operation_count=8; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=y_rounded; child_output=cordic_sincos.y_rounded; operation_count=8; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Status: `failed`
- Severity: `BLOCKER`
- Location: `cordic_sincos.v:67`
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=reduced_phase; child_output=cordic_sincos.reduced_phase; operation_count=6; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=reduced_phase; child_output=cordic_sincos.reduced_phase; operation_count=6; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=reduced_phase; child_output=cordic_sincos.reduced_phase; operation_count=6; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Status: `failed`
- Severity: `BLOCKER`
- Location: `cordic_sincos.v:92`
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=atan_step; child_output=cordic_sincos.atan_step; operation_count=18; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=atan_step; child_output=cordic_sincos.atan_step; operation_count=18; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=atan_step; child_output=cordic_sincos.atan_step; operation_count=18; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Status: `failed`
- Severity: `BLOCKER`
- Location: `cordic_sincos.v:117`
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=state_current; child_output=cordic_sincos.state_current; operation_count=7; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=state_current; child_output=cordic_sincos.state_current; operation_count=7; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=state_current; child_output=cordic_sincos.state_current; operation_count=7; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Status: `failed`
- Severity: `BLOCKER`
- Location: `cordic_sincos.v:126`
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=state_next; child_output=cordic_sincos.state_next; operation_count=7; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=state_next; child_output=cordic_sincos.state_next; operation_count=7; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=state_next; child_output=cordic_sincos.state_next; operation_count=7; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Status: `failed`
- Severity: `BLOCKER`
- Location: `cordic_sincos.v:149`
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=iteration_count; child_output=cordic_sincos.iteration_count; operation_count=7; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=iteration_count; child_output=cordic_sincos.iteration_count; operation_count=7; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=iteration_count; child_output=cordic_sincos.iteration_count; operation_count=7; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Status: `failed`
- Severity: `BLOCKER`
- Location: `cordic_sincos.v:150`
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=protocol_error_result; child_output=cordic_sincos.protocol_error_result; operation_count=4; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=protocol_error_result; child_output=cordic_sincos.protocol_error_result; operation_count=4; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=protocol_error_result; child_output=cordic_sincos.protocol_error_result; operation_count=4; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=x_value; child_output=cordic_sincos.x_value; operation_count=9; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=x_value; child_output=cordic_sincos.x_value; operation_count=9; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=x_value; child_output=cordic_sincos.x_value; operation_count=9; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=y_value; child_output=cordic_sincos.y_value; operation_count=9; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=y_value; child_output=cordic_sincos.y_value; operation_count=9; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=y_value; child_output=cordic_sincos.y_value; operation_count=9; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=sine_result; child_output=cordic_sincos.sine_result; operation_count=0; limit=3; inconclusive_reason=sine_result: missing function definition: $signed; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=sine_result; child_output=cordic_sincos.sine_result; operation_count=0; limit=3; inconclusive_reason=sine_result: missing function definition: $signed; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=sine_result; child_output=cordic_sincos.sine_result; operation_count=0; limit=3; inconclusive_reason=sine_result: missing function definition: $signed; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=z_value; child_output=cordic_sincos.z_value; operation_count=23; limit=3; inconclusive_reason=z_value: missing function definition: $signed; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=z_value; child_output=cordic_sincos.z_value; operation_count=23; limit=3; inconclusive_reason=z_value: missing function definition: $signed; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=z_value; child_output=cordic_sincos.z_value; operation_count=23; limit=3; inconclusive_reason=z_value: missing function definition: $signed; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=cosine_negative; child_output=cordic_sincos.cosine_negative; operation_count=5; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=cosine_negative; child_output=cordic_sincos.cosine_negative; operation_count=5; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=cosine_negative; child_output=cordic_sincos.cosine_negative; operation_count=5; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=cosine_result; child_output=cordic_sincos.cosine_result; operation_count=0; limit=3; inconclusive_reason=cosine_result: missing function definition: $signed; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=cosine_result; child_output=cordic_sincos.cosine_result; operation_count=0; limit=3; inconclusive_reason=cosine_result: missing function definition: $signed; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=cosine_result; child_output=cordic_sincos.cosine_result; operation_count=0; limit=3; inconclusive_reason=cosine_result: missing function definition: $signed; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
...
- Evidence:
- node_kind: `verilog_rtl`
- detail: definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=sine_negative; child_output=cordic_sincos.sine_negative; operation_count=4; limit=3; inconclusive_reason=none; loop_presence=absent
- source_excerpt: `definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=sine_negative; child_output=cordic_sincos.sine_negative; operation_count=4; limit=3; inconclusive_reason=none; loop_presence=absent`
- How to fix: 修复 evidence 所代表的违规事实，并在修改后重新运行对应 VG 门禁。
- Steps:
  1. 打开 location 指向的文件或结构范围，核对 evidence 与当前源码是否一致。
  2. 按 instruction 修改问题片段，保留模块接口、复位和时序契约。
...
- Bad example:
```text
definition_root=cordic_sincos.v:cordic_sincos; instance_path=cordic_sincos; specialization=default; target=sine_negative; child_output=cordic_sincos.sine_negative; operation_count=4; limit=3; inconclusive_reason=none; loop_presence=absent
```
- Good example:
```text
按当前模块接口、时序和可综合约束重写该片段，并保留可追溯的结构事实。
